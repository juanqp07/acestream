...
def try_download_with_gateways(original_url: str, timeout: int = 30) -> str:
    """
    Intenta descargar la URL original; si contiene /ipfs/ o /ipns/ prueba gateways alternativos.
    Orden de intento:
      1) la propia original (si es http/https)
      2) si es ipfs/ipns path, probar reemplazando el host por gateways en ALTERNATIVE_GATEWAYS
      3) si no, prueba gateways intentando simplemente la original y luego los gateways usados con el path si aplica
    """
    parsed = urlparse(original_url)
    path = parsed.path or ""
    is_ipfs_path = "/ipfs/" in original_url or "/ipns/" in original_url

    # 1) intenta original si tiene esquema http/https
    ...
    # 2) si detectamos path ipfs/ipns, construimos con gateways alternativos
    if is_ipfs_path:
        # obtener la parte desde /ipfs o /ipns
        ...
        # si llegamos aquí, intentar la original una vez más (por si no tenía esquema)
        if parsed.scheme == "":
            try:
                print(f"Intentando descargar original sin esquema: {original_url} ...")
                return download_url(original_url, timeout=timeout)
            except RuntimeError as e:
                print(f"Fallo descarga final: {e}")
        raise RuntimeError("No fue posible descargar la URL desde ningún gateway público.")
    else:
        # 3) no parece ipfs/ipns — probamos gateways generando url con path si procede
        for gw in ALTERNATIVE_GATEWAYS:
            # si original tiene path, intentamos reconstruir con gateway base + path
            candidate = gw.rstrip("/") + path if path else gw
            try:
                print(f"Intentando gateway {candidate} ...")
                return download_url(candidate, timeout=timeout)
            except RuntimeError as e:
                print(f"Fallo {candidate}: {e}")
        # intento final con original si no se intentó antes
        try:
